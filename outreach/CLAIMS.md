...
| **Marathi is refused; Nepali is not supported** | They share Hindi's script, so neither hits the error above by script. **Marathi now returns `400 unsupported_language`** — the engine cannot produce `ळ` at all (0 of 4 generations) and substitutes Hindi words for Marathi ones, so it would return audio that is confidently not Marathi. **Nepali still synthesizes**, intelligibly and in Hindi phonology, with an `X-Language-Warning` header; its evidence is weaker (`र्` survived 2 of 4). Say it plainly: "Marathi we refuse rather than fake. Nepali will make a sound and it is not a Nepali voice." Evidence: `eval_out/devanagari/FINDINGS.md`. |
