...
| **Marathi is refused; Nepali is not supported** | They share Hindi's script, so neither is caught by script. **Marathi returns `400 unsupported_language`.** Quote the reason that holds: on every one of four generations the model rendered शाळेत (*shaaLet*, "in school") as शायद (*shaayad*, "maybe") — a different, real **Hindi** word, which is a Hindi lexicon reading Marathi. **Do not lead with "it cannot produce ळ"**: `ळ` never survives our round trip, but it also never appears in any Whisper transcript we have — including from a model that does speak Marathi — so that number cannot separate the engine from the scorer. A native recording would settle it and we have not made one. **Nepali still synthesizes**, intelligibly and in Hindi phonology, with an `X-Language-Warning`; its evidence is weaker (`र्` survived 2 of 4). Evidence: `eval_out/devanagari/FINDINGS.md`. |
